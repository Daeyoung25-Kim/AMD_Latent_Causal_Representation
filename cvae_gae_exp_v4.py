...
class Causal_VAE(tf.keras.Model):
  ...
  def C_ELBO_loss(model, x): #ELBO loss
    ...
    loss1 = tf.norm(z,ord=2)/2
    return mse_loss2(x,x_hat)+loss1/x.shape[0]*0.01
...
while i < Epochs:
    with tf.GradientTape() as dv_t, tf.GradientTape() as dv_t2, tf.GradientTape() as dg_t:
      ...
      cs_l = loss_ + loss_2*2 + alpha*h_a+rho*0.5*tf.math.abs(h_a)**2+lamb*tf.norm(CVAE.GAE.DEC.weights[0], ord=1, axis=[-2,-1]) #weight added
      d_l = loss_2*2 + alpha*h_a+rho*0.5*tf.math.abs(h_a)**2+lamb*tf.norm(CVAE.GAE.DEC.weights[0], ord=1, axis=[-2,-1])
...
plt.plot(loss_of_cv, color="black",label="vae_loss")
#plt.grid()
plt.xlabel('epochs')
plt.ylabel('loss')
plt.legend()
...
d_l #0.43481436371803284

loss_of_cv[-1] #0.0026712417602539062
...
#Hypothesis Testing for Z4
from scipy.stats import spearmanr,kstest,ranksums,mannwhitneyu,brunnermunzel
#kstest(zz[dn_tr==0,4],zz[dn_tr==1,4])
ranksums(zz[dn_tr==0,4],zz[dn_tr==1,4],alternative="less")#np.float64(-2.430862221045956), pvalue=np.float64(0.007531470649496333))
#brunnermunzel(zz[dn_tr==0,4],zz[dn_tr==1,4],alternative="less")

from sklearn.feature_selection import mutual_info_classif, mutual_info_regression
mutual_info_classif(zz[:,4].reshape(-1,1),dn_tr,n_neighbors=3)
...
print(np.min(z11[:,4]),np.max(z11[:,4]))
print(np.min(z11[:,0]),np.max(z11[:,0]))
print(np.min(z11[:,1]),np.max(z11[:,1]))

#Disentanglement simulation
set2 = [-0.4,-0.1,0.2,0.5,0.8] #For Z4
...
#Disentanglement simulation
set1 = [-0.7,-0.5,-0.3,-0.1,0.1] #For ZO
...
plt.imshow(X_nw2[258,:,:,:])#Original image. check location of geographic atrophy
plt.grid()
plt.show()

plt.imshow(X_nw2[154,:,:,:])#Original image. check location of hemorrhage
plt.grid()
plt.show()

plt.imshow(X_nw2[60,:,:,:])#Original image
plt.grid()
plt.show()

plt.imshow(X_nw2[108,:,:,:])#Original image. check location of hemorrhage, drusen
plt.grid()
plt.show()
...
#extracting only highly significant causal links from Weighted adjacency matrix under q=.16

filtered = np.where(np.abs(np.array(CVAE.GAE.DEC.weights[0]))>np.quantile(np.abs(np.array(CVAE.GAE.DEC.weights[0])),0.84),1,0) #
...
filtered_2 = filtered[[1,0,4,8],:] #
filtered_2 = filtered_2[:,[1,0,4,8]]
...
md_c = Sequential([
    InputLayer(shape=(8,)),
    BatchNormalization(),
    Dense(units = 128, kernel_initializer=k3, activation="elu"), #128
    BatchNormalization(),
    Dense(units=64,kernel_initializer=k3,activation="elu"), #64
    BatchNormalization(),
    Dense(units=8,kernel_initializer=k3,activation="elu"), #8
    BatchNormalization(), #
    Dense(units=1,kernel_initializer=k3, activation="sigmoid")
    ])

from tensorflow.keras.optimizers import RMSprop, Adam
md_c.compile(optimizer=Adam(learning_rate=4e-4), loss=tf.keras.losses.BinaryFocalCrossentropy(apply_class_balancing=True,alpha=0.7,label_smoothing=0.4), metrics=["accuracy"]) #0.7, 4e-4, 0.4

hist = md_c.fit(x=z_tr, y=y_n2,epochs=500,batch_size=200)
